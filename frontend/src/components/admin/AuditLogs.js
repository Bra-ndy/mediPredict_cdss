...
const AuditLogs = () => {
  ...
  const fetchAuditLogs = async () => {
    try {
      ...
      // UPDATED: Changed to /auth/admin/audit-logs
  ...
  const applyFilters = (e) => {
    ...
    navigate(`/auth/admin/audit-logs?${params}`);
  };
  ...
  const clearFilters = () => {
    ...
    navigate('/auth/admin/audit-logs');
  };
  ...
  const handleExport = async (format) => {
    try {
      ...
      // UPDATED: Changed to /auth/admin/export-logs
      const response = await api.get(`/auth/admin/export-logs?${params}`, {
        responseType: 'blob'
      });
